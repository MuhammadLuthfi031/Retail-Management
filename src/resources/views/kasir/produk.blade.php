    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <p class="text-xs text-gray-400 mb-4">
                Halaman ini cuma untuk dilihat — buat cek harga per satuan atau sisa stok sebelum/selama layani pembeli.
                Untuk tambah/ubah data produk, hubungi Gudang atau Admin.
            </p>
            ...
            <form method="GET" class="mb-4 bg-white p-4 rounded-lg shadow-sm flex flex-wrap gap-3 items-end">
                ...
                <label class="flex items-center gap-2 text-sm text-gray-600 pb-2">
                    <input type="checkbox" name="show_inactive" value="1" @checked(request('show_inactive'))
                           class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    Tampilkan nonaktif
                </label>
                ...
                @if (request()->anyFilled(['search', 'category_id', 'low_stock', 'show_inactive']))
                    <a href="{{ route('kasir.produk') }}" class="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700">
                        Reset
                    </a>
                @endif
            </form>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-medium text-gray-500 uppercase tracking-wider">Satuan & Harga Jual</th>
                                    ...
                                    @forelse ($product->units as $unit)
                                        <div class="flex items-center gap-1.5 mb-0.5 whitespace-nowrap">
                                            ...
                                            @if ($unit->selling_price !== null)
                                                <span class="text-gray-900">Rp {{ number_format($unit->selling_price, 0, ',', '.') }}</span>
                                            @else
                                                <span class="text-xs text-gray-400 italic">satuan beli saja</span>
                                            @endif
                                        </div>
                                    @empty
